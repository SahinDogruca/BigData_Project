...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Hadoop MapReduce performans değerlendirmesi"
    )
    parser.add_argument("--input", required=True, help="HDFS input path")
    parser.add_argument(
        "--output",
        default=f"{script_dir}/../../results/performance_results.json",
        help="Sonuçları kaydetmek için dosya yolu",
    )
    parser.add_argument(
        "--iterations", type=int, default=3, help="Her test için çalıştırma sayısı"
    )
    parser.add_argument(
        "--sample-sizes",
        type=str,
...
def create_sample_datasets(input_path, sample_sizes):
    ...
    for size in sample_sizes:
        ...
        try:
            ...
            samples[size] = output_path
            print(
                f"Oluşturuldu: {output_path} ({sample_lines} satır, orijinal verinin %{size * 100}'i)"
            )
...
def evaluate_performance(sample_datasets, iterations=3):
    ...
    scripts = {
        f"{script_dir}/../mapreduce/mean_value.py": "Ortalama",
        f"{script_dir}/../mapreduce/max_value.py": "Maksimum",
        f"{script_dir}/../mapreduce/stddev_value.py": "Standart Sapma",
...
def plot_performance(
    results, output_image=f"{script_dir}/../../results/performance_plot.png"
):
